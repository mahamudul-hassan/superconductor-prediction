...
pg = np.load('artifacts/pool_grouped.npz')
y_pool, ylog_pool = pg['y_pool'], pg['ylog_pool']
pred_raw_pool, pred_log_pool = pg['pred_raw_pool'], pg['pred_log_pool']

std_data = np.load('artifacts/std_log_pool_grouped.npz', allow_pickle=True)
std_log_pool = std_data['std_log_pool']
formula_pool = std_data['formula_pool']

cqr = np.load('artifacts/cqr_pool_grouped.npz')
...
for rep in range(N_REPS):
    ...
    # 4. normalized + Mondrian (combined, new method)
...
print(f"\n{'Method':22s} {'Marginal Cov':>13s} {'Avg Width':>10s} {'Group spread':>14s}")
summary = {}
for m in methods:
    ...
    print(f"{m:22s} {oc.mean():11.2f}%  {ow.mean():9.2f}  {spread:12.2f}pp (min={min(gmeans):.1f} max={max(gmeans):.1f})", flush=True)
    summary[m] = {'marginal_coverage_mean': float(oc.mean()), 'marginal_coverage_std': float(oc.std()),
                  'avg_width_mean': float(ow.mean()), 'avg_width_std': float(ow.std()),
                  'group_spread_pp': float(spread),
                  'per_group': {gl: {'cov': float(np.mean(group_cov[m][gl])) if group_cov[m][gl] else None,
                                      'width': float(np.mean(group_width[m][gl])) if group_width[m][gl] else None}
                                for gl in group_labels}}

with open('artifacts/step14_grouped_eval.json','w') as f:
